...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.crf = crf_class.MyCRF(208)  # 添加CRF层
...
def collate_fn(data):
    ...
    for i in data:
        ...
        label = [[206] for _ in range(len_text)]
                ...
                for t in range(len(entity_start)):
                    ...
                    for z in range(entity_start[t], len(item['entity']) + entity_start[t]):
                        if label[z] == [206] and count_a == 0:
                            label[z] = [type2id + 103]
                            count_a += 1
                        elif label[z] == [206] and count_a != 0:
                            label[z] = [type2id]
                        elif label[z] != [206] and count_a == 0:
                            label[z].append(type2id + 103)
                            count_a += 1
                        else:
                            label[z].append(type2id)
        labels.append(label)
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for i, (input_ids, attention_mask, labels) in enumerate(train_loader):
                ...
                out = model.module.crf.viterbi_decode_x(out, attention_mask.to(torch.bool))
                ...
                for j in range(len(labels)):
                    ...
                    for k in range(len(labels[j])):
                        predicted_label = torch.where(out[j][k] > threshold)[0].tolist()  # 将位置索引转换为标签
                        true_label = torch.where(labels[j][k] == 1)[0].tolist()

                        predicted_label_1.append(predicted_label)
                        true_label_1.append(true_label)

                    t_first_index = true_label_1.index([207])
                    t_second_index = true_label_1.index([207], t_first_index + 1)
                    t_modified_label = true_label_1[t_first_index:t_second_index + 1]
                    modified_label = predicted_label_1[t_first_index:t_second_index + 1]
                    predicted_labels.append(modified_label)
                    true_labels.append(t_modified_label)

                    filtered_modified_label = []
                    filtered_t_modified_label = []
                    for idx, label in enumerate(modified_label):
                        if label != [206] or t_modified_label[idx] != [206]:
                            filtered_modified_label.append(label)
                            filtered_t_modified_label.append(t_modified_label[idx])
                    p_list.append(filtered_modified_label)
                    t_list.append(filtered_t_modified_label)

                mlb = MultiLabelBinarizer(classes=list(range(208)))
                y_true = mlb.fit_transform([label for sublist in t_list for label in sublist])
                y_pred = mlb.transform([label for sublist in p_list for label in sublist])
            ...
            if val_f1 > best_val_f1:
                ...
                counter = 0
                torch.save(model.state_dict(), "early_4.pt")
            else:
                counter += 1
